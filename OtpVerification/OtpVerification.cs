...
namespace MhozaifaA.OtpVerification
{
    ...
    public class OtpVerification : IOtpVerification
    {
        ...
        private record class IdPlain(string id,string date,string plain);
        ...
        private bool TryUnprotectUrl(string key, out string id,out string date, out string plain)
        {
        ...
        public OtpVia Generate(string id, string date)
        {
            return Generate(id,date, out _);
        }

        public OtpVia Generate(string id, string date, out DateTime expire)
        {
            return Generate(id,date, options, out expire);
        }

        public OtpVia Generate(string id, string date, OtpVerificationOptions option)
        {
            return Generate(id,date, option, out _);
        }

        public OtpVia Generate(string id, string date, OtpVerificationOptions option, out DateTime expire)
        {
            ...
            if (option.EnableUrl)
                url = BaseOtpUrl + dataProtection.Protect(
                    System.Text.Json.JsonSerializer.Serialize(new IdPlain(id,date, plain)));

            return new OtpVia(plain, url);
        }


        public bool Scan(string id, string date, string plain, OtpVerificationOptions option)
        {
        ...
        public bool Scan(string id, string date, string plain, int expire)
        {
            return Scan(id,date, plain, new OtpVerificationOptions() { Expire = expire });
        }

        public bool Scan(string id, string date, string plain)
        {
            return Scan(id,date, plain, options);
        }

        public bool Scan(string url)
        {
            if (TryUnprotectUrl(url, out string id, out string date,out string code))
                return Scan(id,date, code);
            return false;
        }
    }
}
